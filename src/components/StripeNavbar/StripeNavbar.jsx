...
function StripeNavbar () {
    ...
    function openModalHandler (e) {
        ...
        const bottom = temp.bottom + 40
        openModal(page, {center, bottom})
    }

    function mouseLeaveHandler (e) {
        if(!e.target.classList.contains('w-40')) {
            closeModal()
        }
    }
    ...
    return(
        <>
            <nav className='h-20 flex items-center justify-center bg-transparent relative z-20' >
                <div className='w-[90vw] max-w-[1170px] md:grid md:grid-cols-[auto_1fr_auto] gap-8 md:items-center'>
                            ...
                            stripeSubLinks.map(link => {
                                return (
                                    <li className='md:h-full'>
                                        <button className='md:h-full md:bg-transparent md:border-transparent md:text-lg md:text-white md:w-40 hover:text-stone-600 transition' onClick={openModalHandler}>
                                            {link.page}
                                        </button>
                                    </li>
                                )
